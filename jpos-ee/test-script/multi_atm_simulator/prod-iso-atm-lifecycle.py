...
def pack_msg(mti, fields, macv=None):
    ...
    data = bcd_pack(mti) + bmp.to_bytes(8,"big")
...
def get_response_code(resp):
    ...
    for f in fields:
        if f==39:
            ...
            offset+=1+(l+1)//2
        elif f in [3]: offset+=3
        elif f in [4]: offset+=6
        elif f in [7]: offset+=5
        elif f in [11]: offset+=3
        elif f in [12]: offset+=3
        elif f in [13]: offset+=2
        elif f in [22]: offset+=2
        elif f in [25]: offset+=1
        elif f in [41]: offset+=8
        elif f in [42]: offset+=15
        elif f in [49]: offset+=2
        elif f in [52]: offset+=8
        elif f==62:
...
def build_fields(mti, tid, pan, ksn, key, now, stan):
    fields = {
        2: pan,
        3:"000000",
        4:"000000010000",
    ...
    # Gateway rules: field 52/62 are required for 0200, not allowed for 0100/0400.
    if mti == "0200":
        fields[62] = ksn
        fields[52] = build_pin("1234", pan, key)

    return fields

def send_transaction(mti, fields, with_mac):
    macv=None
    if with_mac:
        temp = pack_msg(mti, fields, macv=b"\x00"*8)
        macv = mac_x919(MAC_KEY, temp)

    body = pack_msg(mti, fields, macv)
    full = TPDU + body
...
def run_term(term, state, with_mac):
    ...
    for _ in range(3):
        ...
        fields_0100 = build_fields("0100", tid, pan, ksn, key, now, stan)

        rc = send_transaction("0100", fields_0100, with_mac)
        print(f"[{tid}] 0100 RC={rc}")

        if rc != "00":
            continue

        fields_0200 = build_fields("0200", tid, pan, ksn, key, now, stan)
        rc = send_transaction("0200", fields_0200, with_mac)
        print(f"[{tid}] 0200 RC={rc}")

        if rc != "00":
            fields_0400 = build_fields("0400", tid, pan, ksn, key, now, stan)
            rc = send_transaction("0400", fields_0400, with_mac)
            print(f"[{tid}] 0400 RC={rc}")

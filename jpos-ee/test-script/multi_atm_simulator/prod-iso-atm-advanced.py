#!/usr/bin/env python3
import os, socket, struct, argparse, json, threading, random, time
...
def send_tx(mti,fields):
    temp=pack_msg(mti,fields,b"\x00"*8)
...
def run_term(term,state):
    ...
    for _ in range(5):
        ...
        simulate_decline=random.random()<DECLINE_RATE

        fields={2:pan,3:"000000",4:"000000010000",
                7:now.strftime("%m%d%H%M%S"),
                11:stan,12:now.strftime("%H%M%S"),
                13:now.strftime("%m%d"),
                22:"051",25:"00",
                41:tid,42:"MERCHANT000001",49:"840"}

        rc=send_tx("0100",fields)
        if simulate_decline: rc=random.choice(["05","51"])
        print(f"[{tid}] 0100 RC={rc}")

        if rc=="TIMEOUT":
            print(f"[{tid}] AUTO REVERSAL (0100)")
            send_tx("0400",fields)
            continue

        if rc!="00": continue

        fields.update({62:ksn,52:build_pin("1234",pan,key)})
        rc=send_tx("0200",fields)
        print(f"[{tid}] 0200 RC={rc}")

        if rc=="TIMEOUT" or rc!="00":
            print(f"[{tid}] AUTO REVERSAL (0200)")
            send_tx("0400",fields)
...
def main():
    ...
    threads=[]
    for t in TERMINALS:
        th=threading.Thread(target=run_term,args=(t,state))
        th.start()
        threads.append(th)
